...
```{r}
#| label: Directory
file_list <- dir_ls(gsub("\\\\", "/", r"(\\oneabbott.com\dept\ADC\Technical_OPS\Clinical_Affairs\CDM_23238\008\UUU)"),recurse = T,glob = "*extracted_realm.zip|*events.csv")
```
...
```{r}
tibble(Path = str_extract(file_path, regex("(?<=Clinical_Affairs/).+|(?<=Upload Data/).+")),
